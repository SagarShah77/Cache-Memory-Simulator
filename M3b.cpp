...
{
  if (dtlb)
    cache_access(dtlb, cmd, addr, NULL, NULL, nbytes, 0, NULL, NULL, FALSE);
  if (cache_dl1)
    cache_access(cache_dl1, cmd, addr, NULL, NULL, nbytes, 0, NULL, NULL, FALSE);
...
sim_main(void)
{
  ...
  while (TRUE)
	...
	cache_access(itlb, Read, IACOMPRESS(regs.regs_PC),
		     NULL, NULL, ISCOMPRESS(sizeof(md_inst_t)), 0, NULL, NULL, FALSE);
      if (cache_il1)
